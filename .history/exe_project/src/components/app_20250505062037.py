...
from ..config.settings import APP_TITLE, DEFAULT_WINDOW_SIZE, COLORS, MESSAGES, VERSION, BUILD_DATE
from ..utils.helpers import center_window, get_os_info
from ..utils.auth import AuthManager
from .ui_components import StyledButton, InfoFrame, LoginFrame
...
class Application:
    ...
    def handle_login(self, username, password):
        ...
        if success:
            ...
            self.login_frame._show_error(result)
    ...
    def handle_register(self, username, password):
        ...
        if success:
            ...
            self.login_frame._show_error(message)
    ...
    def show_about(self):
        ...
        messagebox.showinfo(
            "关于",
            f"{APP_TITLE}\n\n"
            f"版本: {VERSION} (构建于 {BUILD_DATE})\n\n"
            f"这是一个使用Python和Tkinter创建的智能视频分析系统，\n"
            f"可以被打包为Windows可执行文件。"
        ) 
