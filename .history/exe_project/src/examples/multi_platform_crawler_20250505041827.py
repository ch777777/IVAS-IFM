...
def parse_arguments():
    ...
    parser.add_argument('--platforms', '-p', type=str, nargs='+',
                        ...
                        help='要搜索的平台，可选: youtube, bilibili, tiktok, weibo')
    ...
    try:
        ...
        for platform, videos in results.items():
            ...
            for i, video in enumerate(videos):
                ...
                logger.info(f"发布者: {video.get('channel_name', video.get('channel', ''))}")
                ...
                if args.download and video.get('url'):
                    try:
                        ...
                        # 设置文件名
                        filename = f"{platform}_{video.get('video_id')}"
                        ...
                        elif platform == 'tiktok' or platform == 'weibo':
                            file_path = adapter.download_video(
                                video_url=video.get('url'),
                                output_path=output_dir,
                                filename=filename
                ...
                for video in all_videos:
                    ...
                    for key, value in video.items():
                        if isinstance(value, (str, int, float, bool, list, dict, type(None))):
                            ...
                            serializable_video[key] = str(value)
                    serializable_results.append(serializable_video)
