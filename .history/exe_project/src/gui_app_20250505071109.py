...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(str(logs_dir / "ivas-ifm.log"), encoding="utf-8")
    ]
)
...
try:
    ...
    logger.warning(f"无法导入爬虫管理器: {e}，使用模拟数据")
    CrawlerManager = None
...
class App:
    ...
    def setup_window(self):
        ...
        try:
            ...
            # 如果主题不可用，使用默认
            logger.warning(f"主题 {APP_CONFIG['ui']['theme']} 不可用，使用默认主题")
    ...
    def _search_thread(self, query, platforms):
        ...
        try:
            ...
            logger.exception(f"搜索过程中出错: {exc}")
            self.root.after(0, lambda: self._show_error(f"搜索出错: {str(exc)}"))
        finally:
    ...
    def _create_result_item(self, parent, item, platform, row):
        ...
        try:
            frame.configure(style="ResultItem.TFrame")
            self.style.configure("ResultItem.TFrame", relief=tk.GROOVE)
        except Exception:
            # 如果样式设置失败，使用备用方法
            frame = ttk.Frame(parent, padding=10, relief=tk.GROOVE)
            logger.warning("自定义样式配置失败，使用备用样式")
        ...
        download_button = ttk.Button(
            frame,
            text="下载",
            command=lambda url=item['url'], item=item: self.download_video(url, item)
        )
    ...
    def _download_thread(self, url, output_dir, video_data):
        ...
        try:
            ...
            if not file_path:
                raise ValueError("下载失败，未能获取文件路径")
            ...
            logger.exception(f"下载过程中出错: {exc}")
            error_msg = MESSAGES.get('download_error', "下载错误: {error}").format(error=str(exc))
