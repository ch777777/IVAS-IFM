...
class BilibiliAdapter(BaseAdapter):
    ...
    def _parse_search_results(self, content: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        # 使用正则表达式提取视频信息
        pattern = r'<li class="video-item">.*?<a href="//www\.bilibili\.com/video/([^"]+)".*?<img src="([^"]+)".*?<a class="title" href="[^"]+" title="([^"]+)".*?<span class="time">([^<]+)</span>.*?<span class="data">([^<]+)</span>.*?<span class="date">([^<]+)</span>'
        ...
        for match in matches:
            ...
            video_id, thumbnail, title, duration, view_count, upload_date = match.groups()
    ...
    def _parse_video_info(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            description_pattern = r'<div class="desc-info">([^<]+)</div>'
            view_count_pattern = r'<span class="view">([^<]+)</span>'
            like_count_pattern = r'<span class="like">([^<]+)</span>'
            upload_date_pattern = r'<span class="pubdate">([^<]+)</span>'
    ...
    def _parse_upload_date(self, date_str: str) -> str:
        ...
        Args:
            date_str: 日期字符串，格式为 "YYYY-MM-DD" 或 "X天前"、"X周前"、"X月前"、"X年前"
