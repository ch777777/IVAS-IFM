...
class WeiboAdapter(BaseAdapter):
    ...
    def __init__(self, proxy_manager=None):
        ...
        self.search_url = f"{self.base_url}/video"
    ...
    def _parse_search_results(self, content: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        # 使用正则表达式提取视频信息
        pattern = r'<div class="card-feed">.*?<a href="//s\.weibo\.com/video/([^"]+)".*?<img src="([^"]+)".*?<h3 class="txt">([^<]+)</h3>.*?<span class="time">([^<]+)</span>.*?<span class="count">([^<]+)</span>.*?<span class="date">([^<]+)</span>'
    ...
    def _parse_video_info(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            view_count_pattern = r'<span class="count">([^<]+)</span>'
            like_count_pattern = r'<span class="like">([^<]+)</span>'
            upload_date_pattern = r'<span class="date">([^<]+)</span>'
