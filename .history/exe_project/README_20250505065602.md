...
- **Multi-platform Support**: YouTube, TikTok, Bilibili, Weibo, Facebook, etc.
...
| Module | Full Name | Acronym | Function |
|------|----------|------|------|
| Video Crawling | Video Crawling & Aggregation | VCA | Multi-platform video acquisition (YouTube/TikTok/etc.) |
| Multimodal Recognition | Multimodal Content Recognition | MCR | Analyzes video images (CV), speech (ASR), and text (OCR) |
| Semantic Tagging | Semantic Tagging & Metadata Extraction | STME | Generates keywords, scene classification, sentiment analysis |
| Precision Matching | Precision Matching Engine | PME | Associates user search terms with video content tags |
| Material Hub | Advertising Material Hub | AMH | Structured storage of reusable segments |
...
4. Download videos with a single click

### Key Technologies
1. **CV Models**: YOLOv8 (object detection), CLIP (cross-modal image-text matching)
2. **ASR**: Whisper (multilingual speech-to-text)
3. **OCR**: PP-OCRv3 (high-precision text extraction)
4. **PME Logic**: Elasticsearch-based semantic similarity matching
5. **Vector Search**: Using SentenceTransformer to vectorize user searches and video tags
...
- **多平台支持**：YouTube、TikTok、哔哩哔哩、微博、Facebook等
...
| 广告素材库 | Advertising Material Hub | AMH | 结构化存储可复用片段 |
...
### 关键技术
1. **CV模型**：YOLOv8（物体检测）、CLIP（跨模态图文匹配）
2. **ASR**：Whisper（多语言语音转文本）
3. **OCR**：PP-OCRv3（高精度文字提取）
4. **PME匹配逻辑**：基于Elasticsearch的语义相似度匹配
5. **向量搜索**：使用SentenceTransformer向量化用户搜索与视频标签
